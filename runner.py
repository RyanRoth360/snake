...
class Game:
    ...
    def run(self):
        ...
        apple = pygame.mixer.Sound('bite1.wav')
        thud = pygame.mixer.Sound('thud.wav')
        star = pygame.mixer.Sound('star.wav')
        power_down = pygame.mixer.Sound('power_down.wav')
        gold = pygame.mixer.Sound('gold.wav')
        poisen = pygame.mixer.Sound('poisen.wav')
                ...
                while self.running:
                    ...
                    if len(directions)>0:
                        ...
                        if count%30==0:#CONTROLS SNAKE SPEED
                            condition = self.logic.move(directions[-1],self.bounds_power_up)#EXECUTES MOVE
                            if condition=='Delete':#ACCOUNTS FOR FAST TAPS
                                del directions[-1]
                            elif condition=='Crossed' or condition=='OOB':#ENDS GAME
                                star.stop()
                                if self.sound: thud.play()
                    ...
                    if gold_spawned:
                        if pygame.time.get_ticks() > gold_end_time:
                            ...
                            gold_spawned = False
                        elif not self.logic.find_piece('G'):#POWER HAS BEEN EATEN IN TIME
                            if self.sound: apple.play()
                    ...
                    if power_spawned: #IF A BOUNDS POWER WAS SPAWNED 
                        if not self.logic.find_piece('B'): #AND THE POWER IS GONE
    ...
    def _sound_text(self):
        ...
        image_on = pygame.image.load('note_on.png')
        image_on = pygame.transform.scale(image_on,(surface.get_width()*0.15,surface.get_height()*0.1450))
        image_off = pygame.image.load('note_off.png')
        image_off = pygame.transform.scale(image_off,(surface.get_width()*0.15,surface.get_height()*0.1450))
    ...
    def _write_message(self):
        ...
        score_text = font.render('Press R to restart or Q to quit',True,BLACK)
        surface.blit(score_text,(textX,textY))
        ...
        for event in pygame.event.get():
            if event.type==pygame.QUIT:
                ...
                if event.key == pygame.K_LEFT:
                    last_key = 'left'
                    if self.sound: left.play()
                if event.key == pygame.K_RIGHT:
                    last_key = 'right'
                    if self.sound: right.play()
                if event.key == pygame.K_UP:
                    last_key = 'up'
                    if self.sound: up.play()
                if event.key == pygame.K_DOWN:
                    last_key = 'down'
                    if self.sound: down.play()
        return last_key
        ...
        for r in range(15):
            for c in range(15):
                if board[r][c]!=' ':
                    ...
                    rec_height = (surface.get_height()-surface.get_height()*0.08)/15+1
                    ...
                    if board[r][c]=='A':
                        ...
                        if board[r][c]=='S':
                            ...
                            color = pygame.Color(random.randrange(50,255),random.randrange(50,255),random.randrange(50,255))
    ...
    def _get_location(self,r,c):
        #I hard coded this because idk how else to find it
        shift = 0
        if c<5:
            shift = 0.005
        elif c==11:
